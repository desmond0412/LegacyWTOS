...
public class BezierPointEditor : Editor {
	...
	private HandleFunction[] handlers = new HandleFunction[] { HandleMirror, HandleSemiMirror, HandleFree, HandleAbsent };
	...
	void OnSceneGUI()
	{
		...
		Vector3 newPosition = Handles.FreeMoveHandle(point.position, point.transform.rotation, HandleUtility.GetHandleSize(point.position)*0.2f, Vector3.zero, Handles.SphereCap);
		if(point.position != newPosition) point.position = newPosition;
		
		handlers[(int)point.handleStyle](point);
		
		Handles.color = Color.green;
		Handles.DrawDottedLine (point.position, point.globalHandle1,2.0f);
		Handles.DrawDottedLine (point.position, point.globalHandle2,2.0f);
...
//		Handles.DrawLine(point.position, point.globalHandle2);
	...
	private static void HandleMirror(BezierPoint p){
		Handles.color = Color.green;
	...
	private static void HandleSemiMirror(BezierPoint p){
		Handles.color = Color.green;
		...
		if(newGlobal1 != p.globalHandle1){
			...
			float tempLength = (p.globalHandle2 - p.position).magnitude;
			p.globalHandle2 = -(newGlobal1 - p.position).normalized * tempLength + p.position;
		}
		...
		if(newGlobal2 != p.globalHandle2){
			...
			float tempLength = (p.globalHandle1 - p.position).magnitude;
			p.globalHandle1 = -(newGlobal2 - p.position).normalized * tempLength + p.position;
		}
	}

	private static void HandleFree(BezierPoint p){
		Handles.color = Color.green;
