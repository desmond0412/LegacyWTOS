...
{	
	BezierCurve curve;
	SerializedProperty smoothnessProp;
	...
	{	
		...
		Vector3 newPosition = Handles.FreeMoveHandle(point.position, point.transform.rotation, HandleUtility.GetHandleSize(point.position)*0.1f, Vector3.zero, Handles.SphereCap);
		...
		if(point.handleStyle != BezierPoint.HandleStyle.None)
		{
			...
			if(point.globalHandle1 != newGlobal1)
			{
				...
				if(point.handleStyle == BezierPoint.HandleStyle.Mirror) point.globalHandle2 = -(newGlobal1 - point.position) + point.position;
			}
			...
			if(point.globalHandle2 != newGlobal2)
			{
				...
				if(point.handleStyle == BezierPoint.HandleStyle.Mirror) point.globalHandle1 = -(newGlobal2 - point.position) + point.position;
			}
