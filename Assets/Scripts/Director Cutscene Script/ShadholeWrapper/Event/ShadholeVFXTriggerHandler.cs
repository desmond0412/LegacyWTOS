...
[CutsceneItem("Shadhole", "Trigger Event", CutsceneItemGenre.ActorItem)]
class ShadholeVFXTriggerHandler : CinemaActorEvent
{
	public bool isEnable = false;
	public float speed = 0.1f;
	private ShadeHoleVFXHandler ltScript;

	public override void Trigger(GameObject Actor)
	{
		if(Actor != null)
		{
			ltScript = Actor.GetComponent<ShadeHoleVFXHandler>();
			if(ltScript == null )
			{
				Debug.LogError("Actor is does not have Shadhole VFX handler");
				return;                 
			}

			ltScript.SetVFXEnable (isEnable);
		}
	}
}
