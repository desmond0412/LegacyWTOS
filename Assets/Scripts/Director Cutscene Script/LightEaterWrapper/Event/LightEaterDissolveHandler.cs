...
[CutsceneItem("Light Eater", "Dissolve Event", CutsceneItemGenre.ActorItem)]
class LightEaterDissolveHandler : CinemaActorEvent
{
    public bool clear;

    public ShaderAnimationHandler[] shaders;
    ...
    public override void Trigger(GameObject Actor)
    {
        if(Actor != null)
        {
            shaders = Actor.GetComponentsInChildren<ShaderAnimationHandler>();

            if(shaders.Length > 0){
                foreach(ShaderAnimationHandler sh in shaders){
                    sh.TriggerActivation(clear,0.2f);
                }

                diedVFX = Actor.GetComponentInChildren<LightEaterMovementBezier>().diedVFX;
                if(diedVFX){
                    diedVFX.SetActive(!clear);
                }
            }
        }
    }
