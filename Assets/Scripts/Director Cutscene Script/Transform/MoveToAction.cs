...
[CutsceneItem("Transform", "Move To", CutsceneItemGenre.ActorItem)]
class MoveToAction : CinemaActorAction, IRevertable
{
...
//		LerpPosition(Actor,startPosition,moveToPosition.position,transition);
		Actor.transform.position = Vector3.Lerp(startPosition, moveToPosition.position, transition);
		Actor.transform.rotation = Quaternion.Lerp(startRotation, moveToPosition.rotation, transition);
	...
	private void LerpPosition (GameObject Actor, Vector3 positionFrom, Vector3 positionTo, float transition)
	{
		if (Actor != null)
		{
			Actor.transform.position = Vector3.Lerp(positionFrom, positionTo, transition);
		}
	}
}
